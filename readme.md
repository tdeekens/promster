...
<p align="center">
  <b>Promster is a Prometheus Exporter for Node.js servers written for Express, Hapi, Marble.js, Apollo or Fastify.</b>
</p>
...
- [Why another Prometheus exporter?](#-why-another-prometheus-exporter-for-express-and-hapi)
...
- [Getting Started](#-getting-started)
  ...
  - [Exposing metrics via your own server](#exposing-metrics-via-your-own-server)
- [Configuration](#-configuration)
  - [Options](#options)
  - [Customizing buckets and percentiles](#customizing-buckets-and-percentiles)
  - [Custom label values](#custom-label-values)
  - [Request recorder](#request-recorder)
  - [Up/Down signals](#updown-signals)
- [Metrics Reference](#-metrics-reference)
  - [Garbage Collection](#garbage-collection)
  - [HTTP Timings](#http-timings)
  - [GraphQL Timings](#graphql-timings)
...
| [`promster/undici`](/packages/undici)     | [![undici Version][undici-icon]][undici-version]       | [![undici Downloads][undici-downloads]][undici-downloads]       |
...
| [`promster/types`](/packages/types)       | [![types Version][types-icon]][types-version]          | [![types Downloads][types-downloads]][types-downloads]          |

## ❯ Installation

This is a mono repository maintained using
[changesets](https://github.com/atlassian/changesets). It contains
multiple [packages](/packages) including framework integrations (`express`,
...
- `metricNames`: an object containing custom names for one or all metrics with keys of `up, countOfGcs, durationOfGc, reclaimedInGc, httpRequestDurationPerPercentileInSeconds, httpRequestDurationInSeconds`
  - Note that each value can be an `Array<String>` so `httpRequestDurationInSeconds: ['deprecated_name', 'next_name']` which helps when migrating metrics without having gaps in their intake. In such a case `deprecated_name` would be removed after e.g. Recording Rules and dashboards have been adjusted to use `next_name`. During the transition each metric will be captured/recorded twice.
...
- `http_response_content_length_bytes`: a Prometheus histogram with the response content length in bytes (defaults to `[100000, 200000, 500000, 1000000, 1500000, 2000000, 3000000, 5000000, 10000000]`)
  - This metric is disabled by default and can be enabled by passing `metricTypes: ['httpContentLengthHistogram']`.

In addition with each HTTP request metric the following default labels are measured: `method`, `status_code` and `path`. You can configure more `labels` (see [Options](#options)).
...
[types-version]: https://www.npmjs.com/package/@promster/types
[types-icon]: https://img.shields.io/npm/v/@promster/types.svg?style=flat-square
[types-downloads]: https://img.shields.io/npm/dm/@promster/types.svg
