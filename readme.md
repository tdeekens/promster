...
| `signal`              | An `AbortSignal`. Aborting it stops the collection of Garbage Collection metrics. Unset by default, in which case collection runs for the lifetime of the process. Useful to tie collection to a server's shutdown, for instance `process.once('SIGTERM', () => controller.abort())`. Already collected metrics stay in the registry, only further collection stops.                                                                                                                                                                                                                                     |
